...
import { uploadEstatePhoto, deleteEstatePhoto } from '@/lib/actions/estates'
...
interface EstatePhotosSectionProps {
  estateId: string
  photos: Photo[]
}

export function EstatePhotosSection({ estateId, photos }: EstatePhotosSectionProps) {
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    onDropRejected: () => toast.error('Netinkamas failas (max 10MB, tik JPG/PNG/WEBP)'),
  })
  ...
  function handleDelete(photo: Photo) {
    startTransition(async () => {
      try {
        await deleteEstatePhoto(estateId, photo.storagePath)
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <p className="text-sm text-muted-foreground">
          {isDragActive ? 'Paleiskite failus čia…' : 'Vilkite nuotraukas čia arba spustelėkite'}
        </p>
      ...
      {previews.length > 0 && (
        <div className="space-y-3">
          <p className="text-sm font-medium">Laukia įkėlimo: {previews.length} nuotrauka(-os)</p>
          <div className="grid grid-cols-4 gap-2 sm:grid-cols-6 lg:grid-cols-8">
            {previews.map((preview, i) => (
              <div key={i} className="relative group aspect-square">
                <Image src={preview.previewUrl} alt={preview.file.name} fill className="rounded-md object-cover" />
                <Button
                  variant="destructive"
                  size="icon"
      ...
      {photos.length > 0 && (
        <div className="space-y-3">
          <p className="text-sm font-medium">Įkeltos nuotraukos ({photos.length})</p>
          <div className="grid grid-cols-4 gap-2 sm:grid-cols-6 lg:grid-cols-8">
            {photos.map((photo) => (
              <div key={photo.storagePath} className="relative group aspect-square">
                <Image src={photo.url} alt={photo.name} fill className="rounded-md object-cover" />
                <Button
                  variant="destructive"
                  size="icon"
                  className="absolute top-1 right-1 size-7 opacity-0 group-hover:opacity-100 transition-opacity"
      ...
      {photos.length === 0 && previews.length === 0 && (
        <p className="text-sm text-muted-foreground text-center py-4">Nėra įkeltų nuotraukų</p>
      )}
    </div>
  )
}
